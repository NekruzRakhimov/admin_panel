...
func GetNewDefectsPf(req models.DefectsRequest) error {
	return NewFormExcelDefectsPF(req)
}

func GetNewDefectsLs(req models.DefectsRequest) error {
	return NewFormExcelDefectsPF(req)
}
...
func OrderDefectsPF(req models.DefectsRequest) error {
	order, err := repository.CreateDefectsOrder(models.DefectOrder{Date: req.Startdate, Status: "в процессе", IsPf: true})
	if err != nil {
		return err
	}
	...
	if err != nil {
		order.Status = "ошибка при формировании"
		log.Println(err)
	} else {
...
func OrderDefectsLs(req models.DefectsRequest) error {
	order, err := repository.CreateDefectsOrder(models.DefectOrder{Date: req.Startdate, Status: "в процессе", IsPf: false})
	if err != nil {
		return err
	}

	order.FileName, err = FormExcelDefectsLS(req, order.ID)
	if err != nil {
		order.Status = "ошибка при формировании"
		log.Println(err)
	} else {
...
func FormExcelDefectsLS(req models.DefectsRequest, id int) (fileName string, err error) {
	//var restriction int
	f, err := excelize.OpenFile("files/defects/defects_pharmacy_template_ls.xlsx")
	if err != nil {
		return "", err
...
func GetFormedDefectsList(isPf bool) (orders []models.DefectOrder, err error) {
	return repository.GetAllFormedDefects(isPf)
}
